...
public class MoverEnemigo_persigue : AutoEnemigo
{
    ...
    private float rapidezMinima;                       // variable usada para cambiar la rapidez mínima          
    ...
    protected override void Mover()
    {
        ...
        if (contador != 0)                           // si ocurrió una colisión (contador sumó 1, ver abajo)
        {
            ...
            if (contador > espera)                  // si supera la espera, vuelve a su situación original
            {
                ...
                rapidezMinima = minRapidez;             //igual que la rapidez mínima
        ...
        if (jugador.GetComponent<SpriteRenderer>().enabled)
        {
            ...
            if (rapidez < rapidezMinima)                       // sólo si la rapidez llegó al mínimo activa la aceleración
            {
                acelerar = true;
            }
        }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        // si hay colisión con cualquier objeto
            ...
            rapidezMinima = 1f;                                // baja la rapidez mínima, para que no acelere
    }                                           
}
